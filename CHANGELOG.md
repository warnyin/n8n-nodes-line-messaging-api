...
### Changed
- **Clean Production Webhook URLs**
  - Added `isFullPath: true` to webhook configuration
  - Production URL format changed from `/webhook/{workflow-id}/{path}` to `/webhook/{path}`
  - Test URL remains `/webhook-test/{workflow-id}/{path}` for testing purposes
  - Example: Production URL is now `https://n8n.adevtive.com/webhook/line` instead of `https://n8n.adevtive.com/webhook/{workflow-id}/line`

## [0.5.3] - 2025-11-15

### Fixed
- **Path Parameter Now Works**
  - Restored dynamic path expression: `path: '={{$parameter["path"]}}'`
  - Path parameter now properly changes the webhook URL
  - Default path set to "line" instead of empty string
  - Changing path from "line" to other values now updates the URL correctly
